...
exports.sendBotChat = functions.firestore.document('messages/{messageId}').onCreate(
  async (snapshot) => {
    ...
    if (message.length > 0) {
      // Saves the new message into the database
      // which then displays it in the FriendlyChat clients.
      await admin.firestore().collection('messages').add({
        name: 'DimpleBot',
        profilePicUrl: '/images/ekubo.jpg', // Chatbot icon
        text: message,
        timestamp: admin.firestore.FieldValue.serverTimestamp(),
      });
      console.log(`Message starting with ${message.substring(0,maxLogLength)} written to database.`);
    }
  });
